...
internal struct RoomNode
{
    ...
    {
        float incomingHeatCapacity = addedMoles * molarHeatCapacityAtConstantVolume;
        float newHeatCapacity = TotalHeatCapacity + incomingHeatCapacity;
        float newTotalMoles = TotalMoles + addedMoles;
    ...
    public void RemoveGas(int gasId, float removedMoles, float molarHeatCapacityAtConstantVolume)
    {
        float actualRemoved = removedMoles;
        if (GasMoles[gasId] < actualRemoved)
            actualRemoved = GasMoles[gasId];

        float newTotalMoles = MathF.Max(0f, TotalMoles - actualRemoved);
    ...
    private readonly float CalculatePressure(float totalMoles)
    {
